...
import com.mehtank.androminion.server.DominionServer;
 ...
 * How all this works:
 * 
 * if this activity is onCreated without saved instance state and without
 * "command"-"extra", it spawns the startgameactivity, which in turn runs the
 * startgamefragment. the startgamefragment asks the user about parameters of
 * the new game and returns to the startgameactivity, which runs *this* activity
 * again, this time *with* "command".
 * 
 * On creation, we had also created a server intent (DominionServer), which
 * instantiated VDomServer, which started a vdom.comms server and listened on a
 * hardcoded port. when we are run with "command", we send ourselves the
 * STARTGAME event directly, which causes us to connect to the server with the
 * given port, and send it the STARTGAME event. This causes the server to create
 * a thread which contains Game.main() with the arguments we had in this
 * activity's "command" List of Strings. The server sends back a GAMESTATS
 * event.
 * 
 * When the server ran the Game.main(), it gave it a string referencing
 * RemotePlayer.java. RemotePlayer.java is executed by the vdom engine, and it
 * gets all the information that a player would usually get. RemotePlayer also
 * listens on a custom port for someone to connect. This port is reported to us
 * here with the GAMESTATS event. In the function handshake(), we create a class
 * JoinGameDialog, which parses the GAMESTATS and would ask us which position we
 * wanted to join if we had more than one option. We don't, so it uses the
 * option we have and sends us a JOINGAME event. This gives us the port to
 * connect to the RemotePlayer, and the name.
 * 
 * This makes us /disconnect/ from the vdomserver and /connect/ to the
 * RemotePlayer.
 * 
 * We send it a HELLO message, which the RemotePlayer responds to by giving the
 * list of other players and cards. We create a GameTable object, which gives us
 * the game view.
 * 
 * Everything that happens in the game from then on is sent to us as an event,
 * which we handle and respond to. The response contains the information about
 * what we are doing.
 */
...
public class GameActivity extends AppCompatActivity implements EventHandler {
	...
	private Comms comm;
	private volatile boolean readyForMessages = true; // This is set to true
														// when we may receive
														// messages.
	...
	private final ActivityResultLauncher<Intent> activityResultLauncher = registerForActivityResult(
			...
			result -> {
				if (result.getResultCode() == Activity.RESULT_OK) {
					...
					if (data!= null && data.hasExtra("command")) {
						ArrayList<String> strs = data.getStringArrayListExtra("command");
						handle(new Event(EType.STARTGAME).setObject(new EventObject(strs.toArray(new String[0]))));
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			if (getIntent().hasExtra("command")) {
				...
				handle(new Event(Event.EType.STARTGAME).setObject(new EventObject(strs.toArray(new String[0]))));
	...
	public void onResume() {
		...
		WindowInsetsControllerCompat insetsController = ViewCompat.getWindowInsetsController(getWindow().getDecorView().getRootView());
	...
	void startServer() {
		startService(new Intent(this, DominionServer.class));
	}

	void stopServer() {
		stopService(new Intent(this, DominionServer.class));
	}
	...
	protected void alert(String title, String message) {
		new AlertDialog.Builder(this).setTitle(title).setMessage(message).setPositiveButton(android.R.string.ok, (dialog, whichButton) -> dialog.dismiss()).show();
	}
	...
	public boolean handle(Event e) {
		if (!readyForMessages) { // This if false if we expect to be using
									// doWait or get_ts. Then we must return
									// false here so the message is forwarded
									// there.
			return false;
		}
	...
	final Handler mHandler = new Handler(Looper.myLooper()) {
		...
		public void handleMessage(Message msg) {
			...
			case NEWGAME:
				...
				gt.newGame(ng.cards, ng.players, ng.druidBoons, ng.wayOfTheMouseCard);
				gameRunning = true;
				...
				/* TODO frr */
				// Event event = new
				// Event(Event.EType.CARDRANKING).setObject(new
				// EventObject(ng));
				// put(event);
			...
			case STATUS: // RemotePlayer sent us the current status of the game,
							// which we update
				setStatus(e);
				// ack = true;
				break;

			case GETCARD: // RemotePlayer wants us to choose a card and send an
							// EType.CARD event back
				gt.selectCard(e.o.sco, e.s, e.i, e.b);
				break;

			case GETOPTION: // RemotePlayer wants us to choose an option (like
							// the old GETSTRING, but we have to figure out the
							// strings first)
				gt.selectOption(e);
				break;
			...
			// FROM DIALOGS --
			case HELLO: // received from JoinGameDialog; connect to standard
						// port (which would be VDomServer)
				startGame(port);
				break;

			case JOINGAME: // received from JoinGameDialog; connect to received
							// port (would be a RemotePlayer instance)
				name = e.s;
				startGame(e.i);
				break;
			...
			case STARTGAME: // Sent from GameActivity to itself to connect to
							// VDomServer
				if (start(port))
					put(e);
				break;

			// Whenever the user made a choice and sent this to GameTable (or
			// various prompting dialogs), we receive it here.
		...
		private String buildHintString(GameStatus gs, String s, boolean newTurn) {
			...
			List<String> hintStringParts = new ArrayList<String>();
			hintStringParts.add(baseStr);

			if (gs.swampHagAttacks == 1) {
				durationAttacks.add(top.getString(R.string.SwampHagAttacks_single, gs.swampHagAttacks));
			} else if (gs.swampHagAttacks >= 2) {
				durationAttacks.add(top.getString(R.string.SwampHagAttacks_multiple, gs.swampHagAttacks));
			}
			if (gs.hauntedWoodsAttacks) {
				durationAttacks.add(top.getString(R.string.HauntedWoodsAttacks));
			}
			if (gs.enchantressAttacks) {
				durationAttacks.add(top.getString(R.string.EnchantressAttacks));
			}
			if (gs.gatekeeperAttacks) {
				durationAttacks.add(top.getString(R.string.GatekeeperAttacks));
			}
			if (gs.deluded) {
				attackStates.add(top.getString(R.string.DeludedStateEffect));
			}
			if (gs.envious) {
				attackStates.add(top.getString(R.string.EnviousStateEffect));
			}
			if (!durationAttacks.isEmpty())
				hintStringParts.add(TextUtils.join(", ", durationAttacks));
			if (!attackStates.isEmpty())
				hintStringParts.add(TextUtils.join(", ", attackStates));
	...
	private void startGame(int p) {
		...
		try {
			if (!connect(p))
				return;
			// Toast.makeText(top, "Loading game...",
			// Toast.LENGTH_SHORT).show();

			/*
			 * put(new Event(EType.GETSERVER)); Event e = comm.get(); if (e ==
			 * null) throw (new IOException()); if (e.t == EType.SERVER) {
			 * serverName = name; serverHost = e.s; serverPort = e.i; } else
			...
			if (comm == null) { // This is the case when we connect to
								// VDomServer
				return;
			}
	...
	protected void startNewGame(Event e) {
		// if (!NOTOASTS) Toast.makeText(top, "Starting game...",
		// Toast.LENGTH_SHORT).show();
